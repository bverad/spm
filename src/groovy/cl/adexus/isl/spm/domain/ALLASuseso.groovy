...
class ALLASuseso {
	 ...
	 */
	def getXmlDocument(Alla alla){
		
		if( alla==null )return null

		def writer = new StringWriter()
		def xml = new MarkupBuilder(writer)
		def diep
		
		def siniestro = Siniestro.findById(alla?.siniestro?.id)
		if (siniestro?.esEnfermedadProfesional)
			diep = DIEP.findById(siniestro?.diepOA?.id)	
		else
			diep = DIAT.findById(siniestro?.diatOA?.id)
			
		def diagnosticos = Diagnostico.findById(alla?.diagnostico?.id)
		def unMedico = PersonaNatural.findByRun(alla?.medico?.run)
		...
		xml.ALLA {
			...
			ZONA_A() {
				documento() {
					cun(alla.siniestro?.cun)
					folio(alla.id)
					fecha_emision(alla.creadoEl.format("yyyy-MM-dd'T'00:00:00"))
					codigo_org_admin('21')
					codigo_emisor('21')
			...
			ZONA_J() { 
				indicaciones_alta_laboral() {
					((alla.fechaAlta!=null)?fecha_alta_laboral(alla.fechaAlta.format("yyyy-MM-dd")):null)
					((alla.altaInmediata!=null)?alta_inmediata(convertBooleanSiNo(alla.altaInmediata)):null)
					((alla.condiciones!=null)?condiciones(convertBooleanSiNo(alla.condiciones)):null)
					((alla.tipoCondicion!=null)?tipo_condicion(alla.tipoCondicion):null)
					((alla.periodoCondiciones!=null)?n_dias_periodo_condicion(alla.periodoCondiciones):null)
					((alla.continuaTratamiento!=null)?continua_tratamiento(convertBooleanSiNo(alla.continuaTratamiento)):null)
					((alla.tipoTratamiento!=null)?tipo_tratamiento(alla.tipoTratamiento):null)
				} 
			}
			ZONA_L() {
				doctor() {
