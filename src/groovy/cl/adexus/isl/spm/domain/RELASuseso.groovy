...
class RELASuseso {
	...
	def getXmlDocument(Rela rela){
		...
		def siniestro = Siniestro.findById(rela?.siniestro?.id)
		...
		def diagnosticos = Diagnostico.findById(rela?.diagnostico?.id)
		def unMedico = PersonaNatural.findByRun(rela?.medico?.run)
		...
		xml.RELA {
			...
			ZONA_I() { 
				incapacidad_temporal() {
					((rela.inicioReposo!=null)?fecha_inicio_incap_temp(rela.inicioReposo.format("yyyy-MM-dd")):null)
					((rela.terminoReposo!=null)?fecha_termino_incap_temp(rela.terminoReposo.format("yyyy-MM-dd")):null)
					((rela.nDias!=null)?n_dias_incap_temp(rela.nDias):null)
				} 
			}
			ZONA_L() {
				doctor() {
