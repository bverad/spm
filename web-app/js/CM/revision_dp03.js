...
YUI().use("io", "json-parse", "node", "panel", function (Y) {
    Y.on("domready", function () {
        ...
        dialog.onCancel = function (e) {
			...
			Y.one("#modal-content-valor-unitario").detach();
        ...
        dialog.onOK = function (e) {
        	...
        	if (Y.one("#modal-content-clickers-modificar").get("checked") === true) {
        		var monto = Y.one("#modal-content-valor-unitario").get("value").replace(/,/g, '');
		...
		function modificaConsulta (e) {
			...
			var cantidad      = element.get("parentNode").get("parentNode").get("children").item(4).get("text");
			var valorUnitario = element.get("parentNode").get("parentNode").get("children").item(5).get("text").replace(/\.|\,/g, ""); //#948
				...
				on : {
					success : function (tx, r) {
						...
						if (data.respuestaModificarMonto != null) {
							...
							Y.one("#modal-content-modifica-monto").setStyle('display', '');
							Y.one("#modal-content-comment-wrapper").setStyle('width', '40%');
							Y.one("#modal-content-valor-unitario").set("value", data.respuestaModificarMonto);
						} else {
							Y.one("#modal-content-modifica-monto").setStyle('display', 'none');
							Y.one("#modal-content-comment-wrapper").setStyle('width', '95%');
							Y.one("#modal-content-valor-unitario").set("value", valorUnitario);
						}
						Y.one("#modal-content-cantidad-prestaciones").set("value", cantidad);

						var total = Y.one("#modal-content-valor-unitario").get("value").replace(/,/g, '') * cantidad;
						Y.one("#modal-content-valor-total").set("value", total);
						
						// Agregemos su listener acá
						Y.one("#modal-content-valor-unitario").on("change", function () {
							var total = Y.one("#modal-content-valor-unitario").get("value").replace(/,/g, '') * cantidad;
							Y.one("#modal-content-valor-total").set("value", total);
						});
						...
						dialogHeader                = "Consulta Convenio";
						dialog.show();
					}
				}
			});
